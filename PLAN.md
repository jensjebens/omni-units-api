...
| Date | Finding | Impact |
|------|---------|--------|
| 2026-03-25 | Tests must live inside `omni/units_api/tests/` not at extension root `tests/` — Kit's custom importer requires test modules within the python module namespace | Restructured extension layout |
| 2026-03-25 | `scan_for_test_modules = True` in `tests/__init__.py` is required for Kit to discover test classes — eager imports in `__init__.py` fail because `omni.kit.test` isn't loaded yet at extension startup | Changed to lazy scan |
| 2026-03-25 | `Ts.Knot` values default to double — `Ts.Spline("float")` rejects double-valued knots. USD 25.11 is strict about type matching | Used double-typed attr in spline test |
| 2026-03-25 | `--ext-path` works for single extensions, `--ext-folder` doesn't get picked up through `repo.sh test` | Use `--ext-path` for test scripts |
| 2026-03-25 | UsdPhysics declares unit dimensions in doc strings (`Units: mass/distance/distance/distance`) but not as machine-readable metadata — added to proposal body | Strengthens attribute audit call to action |
